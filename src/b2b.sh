...
# basic input
inp=                            # input BLAST result in format 4
out=                            # output
...
SOPT=i:o:w:vh
LOPT=inp:,out:,workdir:erase,verbose,retain,help
hlp() {
    sed <"$DIR/b2b.hlp" -e "s/SCRIPT/$S/g"
}
...
[ -f "$inp" ] || (echo "$S: \"$inp\" is not a valid file." >&2; exit 4)
[ $vbs ] && echo "$S: read from \"$inp\":"
[ "$out" ] || out="${inp%%.*}.bim"
[ $vbs ] && echo "$S: save into \"$out\"."
[ $vbs ] && HR
...
read ltp bp1 qry bp2 <"$inp"    # the 1st one is the query sequence
len=${#qry}
i="0"
j=$bp1
echo $qry | tr - '\n' | while read q; do
    l=${#q}                     # sequence length
    k=$[j+l]                    # ending position
    paste <(seq $j $k) <(echo -n $q | grep -o .)
    j=$k
    #    echo -e "$j\t-"
done | tail
...
exit 0
lt0=sbj
echo -e "QRY_POS\tQRY_SEQ\tSBJ_POS\tSBJ_SEQ"
grep '\(^Subject_1\|^Query_1\)' <"$inp" | while read lt1 bp1 seq bp2; do
    lt1=${lt1:0:3}
    [ lt0 = sbj -a lt1 = que ] && que=seq
    [ lt0 = que -a lt1 = sbj ] && sbj=seq
done
